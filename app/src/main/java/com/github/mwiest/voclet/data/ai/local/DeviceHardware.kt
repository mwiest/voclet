...
class DeviceHardware @Inject constructor(
    ...
    /** The tier recommended for this device for models of [kind]. */
    fun suggestedTier(kind: ModelKind): ModelTier = suggestTierForRam(kind, totalRamBytes())

    companion object {

        /**
         * Pure RAM → tier mapping: the largest model of [kind] the device has
         * the RAM for, read off the catalog rather than restated here.
         * Duplicating the thresholds is how they drift — this pair was 6/12 GiB
         * against models that actually need 10/14, so every 8 GB phone was told
         * to run a model that would thrash it.
         *
         * Per kind, because the two ladders no longer line up: a text model
         * carries no projector, so an 8 GB phone that can only manage the
         * smallest vision model comfortably runs the mid-tier text one.
         *
         ...
         */
        fun suggestTierForRam(kind: ModelKind, totalRamBytes: Long): ModelTier =
            AiModel.forKind(kind)
